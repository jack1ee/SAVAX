...
class HungarianMatcher(nn.Module):
    ...
    def forward(self, outputs, targets, verbose=False, many_to_one=False):
        ...
        with torch.no_grad():
            ...
            out_fine = outputs["mimic_fine_logits"].flatten(0, 1).sigmoid()
            K = out_fine.shape[1]
            ...
            tgt_fines = torch.cat([v["mimic_fine_label"] for v in targets])
            ...
            neg_full = (1 - alpha) * (out_fine ** gamma) * (-(1 - out_fine + 1e-8).log())  # [B*Q, K]
            pos_full = alpha * ((1 - out_fine) ** gamma) * (-(out_fine + 1e-8).log())       # [B*Q, K]
            if K == 1:
                # Binary single-logit case: combine pos/neg costs directly from y in {0, 1}.
                y = tgt_fines
                if y.dim() != 1:
                    y = y.squeeze(-1)
                y = y.to(dtype=out_fine.dtype, device=out_fine.device).clamp_(0, 1)  # [N_t]
                pos = pos_full.squeeze(1)  # [B*Q]
                neg = neg_full.squeeze(1)  # [B*Q]
                cost_fine = pos.unsqueeze(1) * y.unsqueeze(0) + neg.unsqueeze(1) * (1 - y.unsqueeze(0))  # [B*Q, N_t]
            ...
            # Final cost matrix
            C = self.cost_bbox * cost_bbox + self.cost_class * cost_class + self.cost_giou * cost_giou + self.cost_fine * cost_fine
            ...
            if verbose:
                ...
                print(self.cost_fine, cost_fine.var(dim=0), cost_fine.max(dim=0)[0] - cost_fine.min(dim=0)[0])
...
def build_matcher(args):
    cost_fine = getattr(args, "set_cost_fine", 0)

...
from SAVAX.ops.modules import MSDeformAttnCap
...
class Captioner(nn.Module):
    ...
    def build_loss(self, input, target, mask):
        ...
        mask = mask.reshape(-1, mask.shape[-1]).float()

        one_hot = torch.nn.functional.one_hot(target, self.opt.vocab_size + 1).float()
        max_len = input.shape[1]
        output = -(
            one_hot[:, :max_len] * input * mask[:, :max_len, None]
        ).sum(2).sum(1) / (mask.sum(1) + 1e-6)
        return output.reshape(leading_shape)

    def _get_query_mask(self, hs, others, query_mask=None):
        if query_mask is None:
            query_mask = others.get('query_mask')
        if query_mask is None:
            query_mask = others.get('proposals_mask')
        if query_mask is None:
            query_mask = torch.ones(hs.shape[:2], device=hs.device, dtype=torch.bool)
        else:
            query_mask = query_mask.to(device=hs.device, dtype=torch.bool)
        return query_mask

    def _prepare_caption_inputs(self, hs, reference, others, query_mask=None):
        ...
        query_mask = self._get_query_mask(hs, others, query_mask)
        ...
        if n_levels < self.core.opt.num_feature_levels:
            ...
            if input_spatial_shapes.dim() == 1:
                total_input_len = int(input_spatial_shapes.sum().item())
            else:
                total_input_len = int(torch.prod(input_spatial_shapes, dim=-1).sum().item())
    ...
    def forward(self,hs, reference, others, cap_tensor):
        if cap_tensor.dim() == 2:
            seq = cap_tensor.unsqueeze(0)
        else:
            seq = cap_tensor

        batch_size, query_num, _ = hs.shape
        total_queries = batch_size * query_num
        (
            reference_points,
            input_flatten,
            input_spatial_shapes,
            ...
            input_padding_mask,
            query_mask,
        ) = self._prepare_caption_inputs(hs, reference, others)

        state = self.init_hidden(total_queries)
        outputs = []
        seq = seq.long()
        flat_query_mask = query_mask.reshape(-1)

        for i in range(seq.size(-1) - 1):
            step_tokens = seq[:, :, i].reshape(-1)
            if self.training and i >= 1 and self.ss_prob > 0.0: # otherwiste no need to sample
                sample_prob = hs.new_zeros(total_queries).uniform_(0, 1)
                sample_mask = (sample_prob < self.ss_prob) & flat_query_mask
                if sample_mask.sum() == 0:
                    it = step_tokens.clone()
                else:
                    sample_ind = sample_mask.nonzero(as_tuple=False).view(-1)
                    it = step_tokens.data.clone()
                    prob_prev = torch.exp(outputs[-1].data.reshape(total_queries, -1)) # fetch prev distribution: shape Nx(M+1)
                    it.index_copy_(0, sample_ind, torch.multinomial(prob_prev, 1).view(-1).index_select(0, sample_ind))
                    it = Variable(it, requires_grad=False)
            else:
                it = step_tokens.clone()
                # break if all the sequences end
            it = it * flat_query_mask.long()
            if i >= 1:
                valid_tokens = seq[:, :, i][query_mask]
                if valid_tokens.numel() == 0 or valid_tokens.data.sum() == 0:
                    break
            ...
            output = output * flat_query_mask.unsqueeze(1).type_as(output)
            outputs.append(output.reshape(batch_size, query_num, -1))

        if not outputs:
            return hs.new_zeros((batch_size, query_num, 0, self.vocab_size + 1))
    ...
    def sample(self,hs, reference, others, opt=None, query_mask=None):
        ...
        (
            ...
            input_padding_mask,
            query_mask,
        ) = self._prepare_caption_inputs(hs, reference, others, query_mask=query_mask)
        ...
        seq = []
        seqLogprobs = []
        unfinished = flat_query_mask.clone()

        for t in range(self.max_caption_len + 1):
            if t == 0: # input <bos>
                it = hs.data.new(total_queries).long().zero_()
            elif sample_max:
            ...
            logprobs = logprobs * flat_query_mask.unsqueeze(1).type_as(logprobs)

            if t >= 1:
                sampleLogprobs = sampleLogprobs.view(-1) * flat_query_mask.type_as(sampleLogprobs.view(-1))
                # stop when all finished
                if t == 1:
                    unfinished = flat_query_mask & (it > 0)
                else:
                    unfinished = unfinished & (it > 0)
                if unfinished.sum() == 0:
                    break
                step_tokens = (it * unfinished.type_as(it)).reshape(batch_size, query_num)
                step_logprobs = (sampleLogprobs * unfinished.type_as(sampleLogprobs)).reshape(batch_size, query_num)
                seq.append(step_tokens)
                seqLogprobs.append(step_logprobs)

        if not seq:
            empty = hs.new_zeros((batch_size, query_num, 0), dtype=torch.long)
            empty_logprobs = hs.new_zeros((batch_size, query_num, 0))
            return empty, empty_logprobs
        return torch.stack(seq, 2), torch.stack(seqLogprobs, 2)
...
class ShowAttendTellCore(nn.Module):
    ...
    def forward(self,xt, state, query, reference_points, input_flatten, input_spatial_shapes, input_level_start_index, input_padding_mask):
        batch_size, query_num, _ = query.shape
        hidden = state[0][-1].reshape(batch_size, query_num, self.rnn_size)
        joint_query = torch.cat((hidden, query), 2)
        # (N_, N_q, C)
        ...
        att_h = self.h2att(hidden.reshape(-1, self.rnn_size))                    # batch * att_hid_size
        ...
        input_feats = torch.cat((att_res, query.reshape(N_ * Lq_, -1)), 1)
